...
qod %>% count(Cleaned) %>%
  ggplot(aes(y = reorder(Cleaned, n), x = n)) +
  geom_col() +
  ylab("") +
  theme_classic()
...
(gg1 <- ggplot(biketown, aes(x = Distance_Miles)) +
  ...
  annotate(geom= "label",x = mu+2*s, y = 1100, label = "Mean + 2*sd") +
  annotate(geom= "label",x = mu+s, y = 1100, label = "Mean + sd") +
  annotate(geom= "label",x = mu-s, y = 1100, label = "Mean - sd") +
  annotate(geom= "label",x = mu-2*s, y = 1100, label = "Mean - 2*sd") +
...
::: fragment
**Q: Why can't we use mean or SD here?**
:::
...
count(dogs_top5, Dog_Name, Breed) %>%
  tidyr:::pivot_wider(names_from = Breed,
